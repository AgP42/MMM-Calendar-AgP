...
Module.register("MMM-Calendar-AgP", {
	...
	defaults: {
		...
		fetchInterval: 15 * 60 * 1000, // Update every 5 minutes.
		animationSpeed: 2000,
		fade: true,
		urgency: 7,
		timeFormat: "relative",
	...
	updateCalendar: function (){ //was initially on the start function. The setInterval is define on the others js files
	...
	resume: function() { //called by core system when the module is displayed on the screen
		...
		this.updateCalendar();	
//		Log.log("Fct resume - calendar, update interval : " + this.config.fetchInterval);

	},

	notificationReceived: function(notification, payload) {
		if (notification === "USER_PRESENCE") { // notification sended by module MMM-PIR-Sensor or Remote-Control. See its doc
			if (payload){
				this.updateCalendar();
			//	Log.log("User presence true - calendar, update interval : " + this.config.fetchInterval);
			}		
//			Log.log("Fct notificationReceived USER_PRESENCE false calendar (ne fait rien) - payload = " + payload);
		}
	},
	...
	socketNotificationReceived: function (notification, payload) {
		if (notification === "CALENDAR_EVENTS") {
			...
			if(this.config.displayLastUpdate){
				...
				Log.log("Calendar events reçues à : " + moment.unix(this.lastUpdate).format(this.config.displayLastUpdateFormat));
			}
	...
	getDom: function () {
		...
		for (var e in events) {
			...
			if (!this.config.colored) {
				titleWrapper.className = "title bright light";
			} else {
		...
		//ajout de tout ce qui était avant dans le main
		...
		if(this.config.displayLastUpdate){
			...
			MainWrapper.appendChild(updateinfo);
		}
